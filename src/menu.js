...
function setupEventListeners(elements, slides, menus) {
  elements.leftButton.addEventListener("click", () => handleNavigation('prev', elements, slides));
  elements.rightButton.addEventListener("click", () => handleNavigation('next', elements, slides));
  elements.carouselNav.addEventListener("click", e => handleMenuClick(e, elements, slides, menus));
}
...
function handleNavigation(direction, elements, slides) {
  ...
  const targetSlide = direction === 'prev' ? currentSlide.previousElementSibling : currentSlide.nextElementSibling;
  const targetMenu = direction === 'prev' ? currentMenu.previousElementSibling : currentMenu.nextElementSibling;
  const targetIndex = slides.findIndex(slide => slide === targetSlide);

  updateCarousel(elements, slides, currentSlide, targetSlide, currentMenu, targetMenu, targetIndex);
}
...
function updateCarousel(elements, slides, currentSlide, targetSlide, currentMenu, targetMenu, targetIndex) {
...
function updateActiveElements(currentSlide, targetSlide, currentMenu, targetMenu) {
  [currentSlide, currentMenu].forEach(el => el.classList.remove("current-slide"));
